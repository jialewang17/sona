...
def _persona_instructions(persona: str) -> str:
    ...
    persona_map: Dict[str, str] = {
        ...
        "educator": (
            "语气：像科普节目主持人——耐心、有对话感；"
            "可用「其实」「简单来说」「有趣的是」等自然过渡；"
            "分点时写「先说…」「另外…」，不要讲义式条列轰炸或「1)全面回答」模板。"
        ),
        "expert": "语气：专业、克制；术语可保留但需简短解释；结构紧凑但仍像人在讲解。",
...
def _generate_answer(question: str, rows: List[Dict[str, str]], strict_mode: bool, persona: str) -> str:
    ...
    system_prompt = (
        "你是熟悉大熊猫的科普讲解员，正在和提问者自然对话。"
        "只能基于给定知识作答，不得编造。"
        "用流畅的中文段落讲解，像面对面聊天，而不是填表、写报告或列清单。"
        "开头先自然接住用户问题；正文覆盖要点与场景差异（如野外/圈养）；"
        "必要时可加简短小标题（如「气味标记」「声音交流」），但不要用「1)全面回答」「2)依据来源」等编号结构。"
        "不要使用 ✅❌ 等 emoji；不要使用 Markdown 加粗/标题（如 **文字**、# 标题）；避免机械重复同一结论。"
        "正文里不要列出文档路径或「依据来源」清单——来源会由界面另外展示。"
        f"{_persona_instructions(persona)}"
    )
    if strict_mode:
        system_prompt += "当证据不足时，用口语说明「目前资料里还找不到足够依据」，不要硬编。"
    user_prompt = (
        f"问题：{question}\n\n"
        ...
        "请用科普对话体写好回答："
        "先直接回应问题，再分几段讲清楚；"
        "若需分块，用小标题或「先说…」「另外…」等过渡，"
        "结尾可有一句轻量小结或延伸。"
        "若证据不足，直接说明「目前资料里还找不到足够依据」。"
    )
